application.factory('webSqlService', function(){
  ...
  db.transaction(function(tx){
    tx.executeSql('CREATE TABLE IF NOT EXISTS todos (INTEGER PRIMARY KEY, username text, title text, time, done integer)');
  ...
  return {
    insertTodo: function(todo){
      db.transaction(function(tx){
        ...
        tx.executeSql('INSERT INTO todos (username, description, time, done) VALUES (?, ?, ?, ?)', [todo.username, todo.title, 'CURRENT_TIMESTAMP', 0]);
      })
    },
